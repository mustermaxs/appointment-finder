...
function renderAppointment(appointmentId) {
  ...
  $.ajax({
    ...
    success: ({ data }) => {
      ...
      container.append(`<div class="mb-3"><h2>${data.title}</h2> <span style="color:darkred">${expired ? "Voting time is over!" : ""}</span></div>`);

      data.options.map((option) => {
        const isExpired = expired ? 'expired' : '';
        container.append(`
            <div class="doodle-container ${isExpired}-container" id="${option.optionId}" onclick="toggleCheckbox(event, ${option.optionId})">
            <div class="center">
